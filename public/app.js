...
-- The current objective schema could not be loaded.`);
	...
	await builder.loadGraph({
		reset: true,
		message:
			"Builder ready for the current objective schema. Choose a base table and connect related tables to generate SQL.",
	});
...
elements.playgroundSchemaBtn.addEventListener("click", async () => {
	...
	if (result.ok) {
		elements.playgroundSchemaView.textContent =
			result.schema || "Playground has no user tables right now.";
