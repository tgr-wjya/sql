...
export function createVisualSqlBuilder({
  ...
  function buildAdjacency() {
    ...
    for (const edge of graph.foreignKeys) {
      ...
      adjacency.get(edge.fromTable).push({ edgeId: edge.id, target: edge.toTable });
      adjacency.get(edge.toTable).push({ edgeId: edge.id, target: edge.fromTable });
    }

    for (const [key, value] of adjacency.entries()) {
      value.sort((left, right) => {
        const tableDiff = left.target.localeCompare(right.target);
        return tableDiff !== 0 ? tableDiff : left.edgeId.localeCompare(right.edgeId);
      });
  ...
  function reconcileState() {
    ...
    state.selectedEdgeIds = state.selectedEdgeIds.filter((edgeId) => validEdgeIds.has(edgeId));
    const nextJoinTypes = {};
    for (const edgeId of state.selectedEdgeIds) {
      nextJoinTypes[edgeId] = state.joinTypes[edgeId] === "LEFT JOIN" ? "LEFT JOIN" : "INNER JOIN";
    }
    state.joinTypes = nextJoinTypes;
    collectSelectedTables();

    const validColumns = new Set(getAvailableColumns().map((column) => column.key));
    state.selectedColumns = state.selectedColumns.filter((key) => validColumns.has(key));
    state.filters = state.filters.filter((item) => validColumns.has(item.columnKey));
    state.sorts = state.sorts.filter((item) => validColumns.has(item.columnKey));
  }

  function resetState(message = "Builder reset for the current playground schema.") {
    state = createEmptyState();
    state.baseTable = graph.tables[0]?.name ?? "";
    collectSelectedTables();
    render();
    ...
    const html = graph.tables
      ...
      .map((table) => {
        const links = graph.foreignKeys
          .filter((edge) => edge.fromTable === table.name || edge.toTable === table.name)
          .sort((left, right) => describeEdge(left).localeCompare(describeEdge(right)));

        const linkHtml =
          links.length > 0
            ? links
                .map((edge) => {
                  const connected = state.selectedTableNames.includes(table.name);
                  const path = state.baseTable && table.name !== state.baseTable ? findPath(table.name) : null;
                  const disabled = table.name === state.baseTable || connected || path === null;
                  return `<div class="builder-link">
                    ...
                    <button class="btn btn--mini builder-connect-btn" type="button" data-table-name="${escapeHtml(
                      ...
                      connected ? "Selected" : path === null ? "No Path" : "Connect",
                    )}</button>
                  </div>`;
                })
                .join("")
        ...
        const columnSummary = table.columns.map((column) => column.name).join(", ");
        ...
        return `<article class="builder-table ${
          ...
          <div class="builder-table-head">
            ...
            <span class="builder-chip ${
              ...
              table.name === state.baseTable ? "Base table" : state.selectedTableNames.includes(table.name) ? "Included" : "Available",
            )}</span>
          </div>
  ...
  function renderFilters() {
    ...
    elements.builderFilters.innerHTML = state.filters
      .map((filter) => {
        const hideValue = filter.operator === "IS NULL" || filter.operator === "IS NOT NULL";
        return `<div class="builder-row">
          <div class="builder-row-head">
  ...
  function renderPreview() {
    elements.builderSqlPreview.textContent =
      state.generatedSql || "Choose a base table and connect related tables to generate SQL.";

    const messages = [];
    if (graph.tables.length === 0) {
      messages.push("<strong>No tables found.</strong> Run or reset the playground to create a schema first.");
    } else if (graph.foreignKeys.length === 0) {
      messages.push("<strong>No foreign keys detected.</strong> The visual builder needs FK relationships to suggest joins.");
    } else if (!state.baseTable) {
      messages.push("<strong>Pick a base table.</strong> The generated query starts there.");
    } else if (state.selectedColumns.length === 0) {
      messages.push("<strong>Select output columns.</strong> The builder generates SQL once it knows what to project.");
    } else {
      messages.push(`Generated SQL stays editable in the ${targetLabel}. Manual edits pause visual sync.`);
    }
  ...
  function generateSql() {
    ...
    if (state.selectedColumns.length === 0) return "Select at least one output column to generate SQL.";
    ...
    for (const join of buildJoinSequence()) {
      ...
      lines.push(`${joinType} ${join.joinTable} ON ${join.leftExpression} = ${join.rightExpression}`);
    }
    ...
    if (state.sorts.length > 0) {
      lines.push(`ORDER BY ${state.sorts.map((sort) => `${sort.columnKey} ${sort.direction}`).join(", ")}`);
    }
  ...
  async function loadGraph({ reset = false, message = "" } = {}) {
    ...
    if (!result.ok) {
      graph = { tables: [], foreignKeys: [], generatedAt: new Date().toISOString() };
      render();
  ...
  function handleManualInput(nextSql) {
    ...
    setSyncStatus("Manual edits detected. Visual sync paused until you generate and send SQL again.", "info");
  }
  ...
  function bindEvents() {
    elements.builderBaseTable.addEventListener("change", (event) => {
      ...
      collectSelectedTables();
      render();
      setSyncStatus("Base table changed. Reconnect related tables to generate SQL.", "info");
    });
    ...
    elements.builderSelectedJoins.addEventListener("click", (event) => {
      ...
      if (!button) return;
      state.selectedEdgeIds = state.selectedEdgeIds.filter((edgeId) => edgeId !== button.dataset.edgeId);
    ...
    elements.builderFilters.addEventListener("change", (event) => {
      ...
      if (!row) return;
      const filter = state.filters.find((item) => item.id === row.dataset.filterId);
      if (!filter) return;
      if (event.target.matches(".builder-filter-column-select")) filter.columnKey = event.target.value;
      if (event.target.matches(".builder-filter-operator-select")) {
        filter.operator = event.target.value;
        render();
        return;
      }
      if (event.target.matches(".builder-filter-value-input")) filter.value = event.target.value;
    });
    elements.builderFilters.addEventListener("input", (event) => {
      const input = event.target.closest(".builder-filter-value-input");
      if (!input) return;
      const filter = state.filters.find((item) => item.id === input.dataset.filterId);
    ...
    elements.builderFilters.addEventListener("click", (event) => {
      ...
      if (!button) return;
      state.filters = state.filters.filter((item) => item.id !== button.dataset.filterId);
      render();
    });
    ...
    elements.builderSorts.addEventListener("change", (event) => {
      ...
      if (!sort) return;
      if (event.target.matches(".builder-sort-column-select")) sort.columnKey = event.target.value;
      if (event.target.matches(".builder-sort-direction-select")) sort.direction = event.target.value;
    });
    elements.builderSorts.addEventListener("click", (event) => {
      const button = event.target.closest(".builder-remove-sort-btn");
      if (!button) return;
      state.sorts = state.sorts.filter((item) => item.id !== button.dataset.sortId);
      render();
    });
    ...
    elements.builderGenerateBtn.addEventListener("click", () => {
      ...
      renderPreview();
      setSyncStatus("SQL generated from the current visual builder state.", "ok");
    });
    ...
    elements.builderSendBtn.addEventListener("click", () => {
      ...
      renderPreview();
      setSyncStatus(`Generated SQL sent to the ${targetLabel}. Manual edits will pause visual sync.`, "ok");
    });
    ...
    elements.builderClearBtn.addEventListener("click", () => {
      resetState("Builder cleared. Choose a base table and reconnect tables when ready.");
    });
  }
