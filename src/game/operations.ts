				...
				validate: (db) => {
					...
					if (!hasRangeCheck) {
						return {
							pass: false,
							detail:
								"Table 'employees' must include CHECK(clearance_level BETWEEN 1 AND 5).",
						};
					}
					...
					const pkCheck = hasCompositePk(db, "employee_projects", [
						"employee_id",
						"project_id",
					]);
				...
				validate: (db) => {
					...
					if (!indexExists) {
						return {
							pass: false,
							detail:
								"Required index idx_employee_projects_project_id was not created.",
						};
					}

					const plans = db
						.query(
							"EXPLAIN QUERY PLAN SELECT * FROM employee_projects WHERE project_id = 201;",
						)
						.all() as Array<{ detail: string }>;

					const usesIndex = plans.some((plan) =>
						plan.detail.includes(
							"USING INDEX idx_employee_projects_project_id",
						),
					);
...
`,
				validate: (db) => {
					...
					const planRows = db
						.query(
							"EXPLAIN QUERY PLAN SELECT * FROM transfers WHERE employee_id = 7;",
						)
						.all() as Array<{ detail: string }>;
					...
					if (!indexExists) {
						return {
							pass: false,
							detail: "Missing required index idx_transfers_employee.",
						};
					}
