...
function compareResultSets(
  ...
  const expectedCols = expectedColumns.length > 0 ? expectedColumns : extractColumns(expectedRows, []);
  const actualCols = actualColumns.length > 0 ? actualColumns : extractColumns(actualRows, []);
  ...
  const expectedValues = expectedRows.map((row) => valuesForColumns(row, columns));
...
function executeSql(db: Database, sql: string): { rows: SqlRow[]; columns: string[]; lastIsQuery: boolean } {
...
export class GameEngine {
  ...
  private getCurrent(state: GameState): { operation: (typeof OPERATIONS)[number]; objective: Objective } | null {
  ...
  run(sql: string): RunResponse {
    ...
    if (current.objective.requiredTokens && current.objective.requiredTokens.length > 0) {
      const requiredCheck = hasRequiredTokens(cleaned, current.objective.requiredTokens);
    ...
    try {
      if (current.objective.mode === "result") {
        ...
        const expectedExecution = executeSql(expectedDb, current.objective.solutionSql ?? "");
        expectedDb.close();
    ...
    if (!alreadySolved) {
      const hintsUsed = state.hintsUsed[key] ?? 0;
      xpAwarded = Math.max(5, current.objective.xp - hintsUsed * HINT_PENALTY_XP);
      state.xp += xpAwarded;
  ...
  hint(): { ok: boolean; message: string; hint: string | null; snapshot: Snapshot } {
    const state = this.progressStore.load();
    const current = this.getCurrent(state);
  ...
  advance(): { ok: boolean; message: string; snapshot: Snapshot } {
    ...
    const isLastObjectiveInOperation = state.objectiveIndex >= operation.objectives.length - 1;
